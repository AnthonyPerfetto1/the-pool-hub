// Mirrors server/src/utils/date-ranges.ts's getWeekRange and
// apps/mobile/src/lib/business-week.ts exactly — same IANA zone, same
// guess-and-correct offset algorithm — so the web Schedule's week/day
// boundaries never disagree with the backend's own Monday-Sunday reporting
// weeks or the mobile Schedule. There is no shared runtime module between
// the server and either client (packages/types is types-only), so this is a
// deliberate, minimal port; keep all three implementations in sync if any
// one of them changes.
...
// key — used to identify "today" without any browser-timezone dependency.
...
export function formatWeekdayShort(instant: Date): string {
  return new Intl.DateTimeFormat("en-US", { timeZone: BUSINESS_TIME_ZONE, weekday: "short" })
    .format(instant)
    .toUpperCase();
}
...
export function formatMonthDayLong(instant: Date): string {
  return new Intl.DateTimeFormat("en-US", { timeZone: BUSINESS_TIME_ZONE, month: "long", day: "numeric" }).format(
    instant,
  );
}

export function formatYear(instant: Date): string {
  return new Intl.DateTimeFormat("en-US", { timeZone: BUSINESS_TIME_ZONE, year: "numeric" }).format(instant);
}
